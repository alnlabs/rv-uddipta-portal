...
export function canEditBuilder(role: AppRole | null | undefined, user: User | null) {
  return role === "admin" || role === "builder" || isAdminUser(user);
}

export function canManageAdmin(role: AppRole | null | undefined, user: User | null) {
  return role === "admin" || isAdminUser(user);
}
...
export async function ensureProfile(
  ...
  if (existing) {
    if (isAdminUser(user) && existing.role !== "admin") {
  ...
  const admin = createAdminClient();
  let role: AppRole = isAdminUser(user) ? "admin" : "visitor";
